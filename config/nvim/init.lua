...
-- - emmet
--   - or surround for wrapping in tag
-- - Window zoom
...
vim.api.nvim_create_autocmd("InsertEnter", {
    callback = function()
        if layout == 1 then
            local handle = io.popen("hyprctl switchxkblayout keyd-virtual-keyboard 1")
            handle:close()
        end
    end,
})
...
vim.api.nvim_create_autocmd("InsertLeave", {
    callback = function()
        local handle = io.popen("hyprctl devices | grep 'active keymap: Russian'")
        ...
        local handle = io.popen("hyprctl switchxkblayout keyd-virtual-keyboard 0")
        handle:close()
    end,
})
...
require("lazy").setup({
    ...
    spec = {
            ...
            config = function()
                require("auto-save").setup({
                    ...
                    condition = function(buf)
                        ...
                        if filetype == "gitcommit" or filetype == "oil" then
                            return false
                        end

                        local filepath = vim.fn.expand('%:p')
                        if filepath:match('^scp://') then
                            -- Remote editing is too slow to auto-save
                            return false
                        end

                        if filepath:match('^/tmp/') then
                            -- It's usually something like git commit message,
                            -- `sudo -e` or zsh buffer edit
                            return false
                        end
        ...
        -- Breadcrumbs
        {
            ...
            event = "VeryLazy",
            -- enabled = false, -- FIXME: for video recording
            config = function()
